...
class PdoGsb {
    ...
    public function getLesVisiteursAValider() {
        $req = "SELECT v.nom, v.prenom, f.idEtat, DATE_FORMAT(f.dateModif, '%d/%m/%Y') AS dateModif , f.idVisiteur FROM visiteur v INNER JOIN fichefrais f ON v.id=f.idVisiteur
                WHERE f.idEtat = 'CL'";
        $res = $this->monPdo->query($req);
